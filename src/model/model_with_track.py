...
def train(filename):
    ...
    # Start MLflow run
    mlflow.set_experiment("MovieReDial_QA_System")  # Set the name of the experiment
    with mlflow.start_run():
        # Log the current date as a parameter
        mlflow.log_param("current_date", datetime.now().strftime("%Y-%m-%d"))
        ...
        # Log metrics
        mlflow.log_metric("average_loss", total_loss / len(loader))
...
if __name__ == "__main__":
    filename = '../../data/processed/redial_dataset_train.json'  # Adjust the filename path as needed
    train(filename)
